...
<body>
	<div style="width:100%; height:10vh; display:flex; align-items:center; ">
		<h3 style="margin: 0px auto; wiodth:300px;">Orders</h3>
	</div>
	<div id="left-pane" style="width:100%; height:87vh; float:left; display:flex; overflow-y:auto; ">
		...
		<table class="table table-hover" style="margin:0px auto; width:90%;">
			<tr class="success"><th>#</th><th>Customer</th><th>Contact</th><th>Email</th><th>Address</th><th>Amount</th><th>Date</th><th>Items</th><th>Delivery Person</th><th>Delivery Area</th></tr>
			<?php
			$res=$db->getOrders();
			...
			while($row = mysqli_fetch_assoc($res))
			{
				...
				echo '<button id="submitSection1" type="submit" name="detail" value="detail" class="btn btn-success">Order Detail</button>';
				echo '</form></td>';
				echo '<td><form method="post"enctype="multipart/form-data" action="orderassign.php">';
				if(!is_null($row['person_name'])){
					echo '<button id="submitSection1" type="submit" name="assign" value="assign" class="btn btn-danger" disabled>Assigned</button>';		
				}else{
					echo '<input type="hidden" name="order_id" value="'.$row['order_id'].'">';
